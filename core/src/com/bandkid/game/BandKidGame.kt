...
class BandKidGame : ApplicationAdapter() {
    ...
    override fun create() {
        ...
        font = FreeTypeFontGenerator(files.internal(COUNTDOWN_FONT)).let {
        ...
        dropSound = audio.newSound(files.internal(DROP_SOUND))
        rainMusic = audio.newMusic(files.internal(RAIN_SOUND))
    ...
    override fun render() {
        ...
        font.draw(batch, "${(1/ graphics.deltaTime).toInt()}", 700f, 400f)
        batch.end()

        for (drop in raindrops) {
            drop.y -= 200 * graphics.deltaTime
    ...
    private fun moveRectangle(item: Rectangle) {
        when {
            input.isTouched -> {
                Vector3(input.x.toFloat(), input.y.toFloat(), 0f).let {
                    camera.unproject(it)
                    item.x = it.x - 64 / 2
                }
            }
            input.isKeyPressed(LEFT) -> item.x -= 200 * graphics.deltaTime
            input.isKeyPressed(RIGHT) -> item.x += 200 * graphics.deltaTime
        }
    }
